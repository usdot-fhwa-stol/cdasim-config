...
class MoveOverLaw:
    ...
    def __init__(self, sumo_connector, veh_id):
        ...
        self._park_area_id = config.get('Settings', 'park_area_id')
    ...
    def park_messenger(self):
        ...
        if not self.init_park:
            self.sumo_connector.park_area_stop(self._veh_id, self._park_area_id)
            self.init_park = True

        elif self.sumo_connector.get_veh_speed(self._veh_id) == 0:
            self.close_lane()
